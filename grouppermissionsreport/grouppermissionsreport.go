...
func SpacePermissionsReport(propPtr string) {
	...
	if cfg.Simple {
		...
		for _, group := range Groups {
			if schedulerutil.CheckScheduleDetail("SpacePermissionsReport-" + group, 7*time.Hour*24,cfg.Reset, schedulerutil.DummyFunc,"jiracategory.properties") {
				cfg.Group = group
...
func CreateGroupPermissionsReport(cfg ReportConfig) {
	...
	max := 2
	for cont {
		groupMembers, err := theClient.GetGroupMembers(cfg.Group, &client.GetGroupMembersOptions{StartAt: start, MaxResults: max, ShowExtendedDetails: true})
		if err != nil {
			panic(err)
		}
